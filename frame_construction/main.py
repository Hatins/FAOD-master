...
dataset_2_height = {'pku_fusion': 260}
dataset_2_width = {'pku_fusion': 346}
...
def prophesee_bbox_filter(labels: np.ndarray, dataset_type: str) -> np.ndarray:
    assert dataset_type in {'pku_fusion'}
...
if __name__ == '__main__':
    ...
    parser.add_argument('--input_dir', default='/data/xiangyuanw/PKU_FUSION/PKU-H5/')
    parser.add_argument('--target_dir',
                        default='/data/xiangyuanw/PKU_FUSION/PKU-H5-Process')
    ...
    parser.add_argument('--bbox_filter_yaml_config',
                        default='frame_construction/conf_preprocess/filter_pku_fusion.yaml',
                        help='Path to bbox filter yaml config file')
    parser.add_argument('-ds', '--dataset', default='pku_fusion', help='pku_fusion or dsec')
    parser.add_argument('-np', '--num_processes', type=int, default=10, help="Num proceesses to run in parallel")

    # parser.add_argument('-ev_divide_N', '--event_divide_number', type=float, default=8, help = 'ev_hz = 25 * args.event_divide_number if 25 * args.event_divide_number > 1  \
    #                     img_hz = round(25 * args.event_divide_number, 2) if args.event_divide_number < 1')
    parser.add_argument('--image_upsampling_rate', type=float, default=0.1, help='assert <= 1')
    parser.add_argument('--event_upsampling_rate', type=float, default=10, help='assert >= 1')
    ...
    parser.add_argument('--pku_scenes', type=list, default=['normal', 'low_light', 'motion_blur'], help="")
    parser.add_argument('--pku_splits', type=list, default=['test'], help="")
    ...
    scenes = args.pku_scenes    # ['normal', 'low_light', 'motion_blur']
    ...
    for split in split_path_list:   # [train_path, val_path, test_path]:
        # split_normal_scene = split / 'normal'
        # split_low_light_scene = split / 'low_light'
        # split_motion_blur_scene = split / 'motion_blur'
        split_scene_list = []
        for scene in scenes:
            split_scene_list.append(split / scene)

        for subsplit in split_scene_list:   # [split_normal_scene, split_low_light_scene, split_motion_blur_scene]:
            if args.save_each_scenes:
                split_out_dir = target_dir / '/'.join(subsplit.parts[-2:])
            else:
                split_out_dir = target_dir / subsplit.parts[-2]     # os.path.join(str(target_dir), str(subsplit.parts[-2:]))
            os.makedirs(split_out_dir, exist_ok=True)
            for npy_file in subsplit.iterdir():
