...
from data.ev_img_dataloader.labels import ObjectLabels
from data.utils.types import DataType, LstmStates, ObjDetOutput, DatasetSamplingMode
from models.detection.yolox.utils.boxes import postprocess
from models.detection.yolox_extension.models.detector_fusion import YoloXDetector
...
class Module(pl.LightningModule):
    def __init__(self, full_config: DictConfig):
        ...
        self.enable_align = self.mdl_config.backbone.enable_align
        self.align_loss = self.mdl_config.backbone.using_align_loss
    ...
    def training_step_with_ssm(self, batch: Any):
        ...
        image_tensor_sequence = torch.stack(
            image_tensor_sequence
        )  # shape: (sequence_len, batch_size, channels, height, width) = (L, B, C, H, W)

        ev_tensor_sequence = ev_tensor_sequence.to(dtype=self.dtype)
        image_tensor_sequence = image_tensor_sequence.to(dtype=self.dtype)

        ev_tensor_sequence = self.input_padder.pad_tensor_repr(ev_tensor_sequence)
        image_tensor_sequence = self.input_padder.pad_tensor_repr(image_tensor_sequence)
        ...
        # loss_aligned = loss_aligned * 10
        # print('-----------loss_aligned:', loss_aligned)
        prev_states = states
    ...
    def training_step_with_rnn(self, batch: Any):
        ...
        feature_alighed_list = list() #using for align loss
        feature_unalighed_list = list()
        
        for tidx in range(sequence_len):
            ev_tensors = ev_tensor_sequence[tidx]
            img_tensor = image_tensor_sequence[tidx]
            ...
            if self.align_loss == True:
                assert self.enable_align == True
                
            backbone_features, states, _, feature_alighed, feature_unalighed = self.mdl.forward_backbone_rnn(ev_input=ev_tensors,
                                                                  img_input=img_tensor,
                                                                  previous_states=prev_states,
                                                                  token_mask=token_masks)
                
            feature_alighed_list.append(feature_alighed)
            feature_unalighed_list.append(feature_unalighed)
        ...
        if self.align_loss == True:
            ...
            if drift is not None:
                for bb in range(batch_size):
                    img_input_aligned_bb = torch.stack(feature_alighed_list[:(sequence_len-drift[bb])], dim=0)[:, bb]      # [L1, C, H, W]
                    img_input_unaligned_bb = torch.stack(feature_unalighed_list[-(sequence_len-drift[bb]):], dim=0)[:, bb] # [L1, C, H, W]
    ...
    def _val_test_step_impl_with_ssm(self, batch: Any, mode: Mode) -> Optional[STEP_OUTPUT]:
        ...
        if self.mode_2_hw[mode] is None:
            assert ev_tensor_sequence.shape[-2:] == image_tensor_sequence.shape[-2:]
            self.mode_2_hw[mode] = tuple(ev_tensor_sequence.shape[-2:])
        else:
            assert ev_tensor_sequence.shape[-2:] == image_tensor_sequence.shape[-2:]
            assert self.mode_2_hw[mode] == ev_tensor_sequence.shape[-2:]

        backbone_features, states = self.mdl.forward_backbone_ssm(ev_input=ev_tensor_sequence, img_input=image_tensor_sequence, 
                                                                  previous_states=prev_states, train_step=False)
        prev_states = states
    ...
    def _val_test_step_impl_with_rnn(self, batch: Any, mode: Mode) -> Optional[STEP_OUTPUT]:
        data = self.get_data_from_batch(batch)
        worker_id = self.get_worker_id_from_batch(batch)
        ...
        for tidx in range(sequence_len):
            ...
            if self.mode_2_hw[mode] is None:
                assert ev_tensors.shape[-2:] == img_tensor.shape[-2:]
                self.mode_2_hw[mode] = tuple(ev_tensors.shape[-2:])
            else:
                assert ev_tensors.shape[-2:] == img_tensor.shape[-2:]
                assert self.mode_2_hw[mode] == ev_tensors.shape[-2:]

            backbone_features, states = self.mdl.forward_backbone_rnn(ev_input=ev_tensors, img_input=img_tensor, previous_states=prev_states)
            prev_states = states
    ...
    def test_step(self, batch: Any, batch_idx: int) -> Optional[STEP_OUTPUT]:
        if self.memory_type in ['lstm', 'gru', 'rnn']:
            return self._val_test_step_impl_with_rnn(batch, mode=Mode.TEST)
        elif self.memory_type in ['s4', 's5', 's6']:
