...
def main(config: DictConfig):
    ...
    rgb_file = h5py.File(rgb_file)
  
    event_frames = ev_file['data']
    rgb_frames = rgb_file['data'][1:]
    ...
    for frame_index in tqdm(range(event_frames.shape[0])):
        ...
        rgb_frame = torch.tensor(rgb_pr).permute(2,0,1).unsqueeze(0)

  
        event_frame = event_frame.to(gpus)
        rgb_frame = rgb_frame.to(gpus)
        ...
        if mode == 'gt':
            ...
            with torch.inference_mode():
                results, pre_state = module.forward(event_frame, rgb_frame, pre_state)
                results = results[0]
        if results is None:
            results = []
        ...
        if len(results) > 0: 
            for each_bbox in results: 
                ...
                if mode == 'gt':
                    ...
                    if label_id == 0 or label_id == 1 or label_id == 2 or label_id == 3 or label_id == 4:
                        color = bbox_color
                    else:
                        color = unknown_color
